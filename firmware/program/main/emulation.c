...
#define SERVER_NAME    "http://127.0.0.1"
#define SERVER_PATH    "/platerecog"
...
static esp_err_t send_image_to_api(const uint8_t *image_data,
    ...
    int  total_len =
        strlen(boundary_start) + strlen(content_disposition) + image_len + strlen(boundary_end);

    char total_len_str[16];
    sprintf(total_len_str, "%d", total_len);
    ...
    esp_http_client_write(client, boundary_start, strlen(boundary_start));
    ...
    if (status_code == 200 && content_length > 0) {
        ...
        if (response_buffer) {
            ...
            if (root) {
                cJSON *json_plate   = cJSON_GetObjectItem(root, "number_plate");
                cJSON *json_access  = cJSON_GetObjectItem(root, "access");
                cJSON *json_payment = cJSON_GetObjectItem(root, "payment_status");

                if (cJSON_IsString(json_plate))
                    plate_num = strdup(json_plate->valuestring);

                *access_granted =
                    (json_access && strcmp(json_access->valuestring, "granted") == 0)
                    || (json_payment && strcmp(json_payment->valuestring, "paid") == 0);
